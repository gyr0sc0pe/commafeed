		...
		if (StringUtils.isBlank(categoryId)
			...
			if (parent != null) {
				List<FeedCategory> categories = feedCategoryDAO
						.findAllChildrenCategories(user, parent);
				List<FeedSubscription> subs = feedSubscriptionDAO
						.findByCategories(user, categories);
				statuses = feedEntryStatusDAO.findUnreadBySubscriptions(user,
						subs, null, 0, 1, order, true);
			}
		}
